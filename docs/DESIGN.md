...
| Thin CLI (`src/cli.ts`, npm `agentwiki`) | Shells out to `claude -p` headless mode for terminal/CI use | User's subscription (local login) or `CLAUDE_CODE_OAUTH_TOKEN` from `claude setup-token` (CI) |
...
- **Incremental updates** driven by `agentwiki.json` metadata (last documented commit + format), diff-scoped.
- **CI workflow** example for GitHub Actions (`examples/agentwiki-update.yml`).
...
├── commands/                    # /agentwiki:init, /agentwiki:update
